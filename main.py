...
async def update_user(
    ...
    if user_update.email is not None and user_update.email != user.email:
        result = await db.execute(
            await select(models.User).where(models.User.email == user_update.email),
        )
...
async def update_post_parcial(post_id: int, post_data: PostUpdate, db: Annotated[AsyncSession, Depends(get_db)]):
    result = await db.execute(
        select(models.Post)
        .options(selectinload(models.Post.author))
        .where(models.Post.id == post_id))
